...
export default async function NewProspect({ searchParams }: { searchParams: Promise<{ season?: string }> }) {
 ...
 {season?.status === "active" ? <ProspectForm seasonId={season.id} year={season.year} /> : <div className="panel mt-6 p-6"><p>Historical seasons are read-only. Choose an active season to create a prospect.</p><Link className="mt-4 block text-primary underline" href="/prospects">Back to prospects</Link></div>}</div>;
}
