...
export default async function EditProspect({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ season?: string }> }) {
 ...
 {season?.status === "active" ? <ProspectForm prospect={person} seasonId={season.id} year={season.year} /> : <div className="panel mt-6 p-6"><p>Historical seasons are read-only. Choose an active season to edit shared player facts.</p><Link className="mt-4 block text-primary underline" href={"/prospects/" + person.id + (season ? "?season=" + season.year : "")}>Back to profile</Link></div>}</div>;
}
