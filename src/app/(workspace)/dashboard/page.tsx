...
export default async function Dashboard({ searchParams }: { searchParams: Promise<{ season?: string }> }) {
  ...
  return <div className="space-y-9">
    ...
    <section aria-labelledby="pipeline-title"><div className="mb-4 flex items-center justify-between"><h2 id="pipeline-title" className="text-sm font-semibold">The recruiting pipeline</h2><span className="text-xs text-muted-foreground">{season?.name ?? "Select a season"}</span></div><div className="grid gap-4 md:grid-cols-3">{stages.map(({ name, caption, icon: Icon, tone }) => <article key={name} className="panel p-6"><div className="flex items-center justify-between"><span className="text-xs font-semibold text-muted-foreground">{name}</span><div className={`rounded-md p-2 ${tone}`}><Icon size={16} /></div></div><p className="mt-5 text-4xl font-medium tracking-tight text-slate-400" aria-label="Count not connected">—</p><p className="mt-3 text-xs leading-5 text-muted-foreground">{caption}</p></article>)}</div><p className="mt-3 text-[11px] text-muted-foreground">Pipeline counts will connect when prospect tracking is implemented.</p></section>
    <section className="foundation-banner blueprint-grid" aria-labelledby="foundation-title"><div className="relative z-10 max-w-xl"><span className="eyebrow text-blue-200">Phase 1 / Workspace foundation</span><h2 id="foundation-title" className="mt-4 text-2xl font-semibold tracking-tight">The workspace is taking shape.</h2><p className="mt-3 text-sm leading-6 text-blue-100/80">Navigation, leadership access, and season context are in place. Prospect records, notes, and evaluations are not available yet. No player data has been imported.</p><Button asChild variant="outline" className="mt-6"><Link href={`/prospects${query}`}>View prospect workspace <ArrowRight size={15} /></Link></Button></div><ScanLine size={112} strokeWidth={0.65} className="absolute right-10 top-1/2 hidden -translate-y-1/2 text-blue-200/30 xl:block" /></section>
    <div className="grid gap-4 md:grid-cols-2"><section className="panel p-6"><span className="eyebrow text-muted-foreground">One shared workspace</span><h2 className="mt-3 text-base font-semibold">Keep leadership on the same page.</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Active captains and coaches share the same access. Membership is managed privately through Supabase.</p></section><section className="panel p-6"><span className="eyebrow text-muted-foreground">Beyond a single season</span><h2 className="mt-3 text-base font-semibold">Build for the long game.</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Start in 2027 or switch to 2026. Season selection is ready; cross-season prospect history comes in a later phase.</p></section></div>
  </div>;
}
