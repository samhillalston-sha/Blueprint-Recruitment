...
import { evaluationAttributes } from "@/lib/evaluation-policy";
...
export function ActivityTimeline({ rows, count, page, seasons, profileUrl }: { rows: Activity[]; count: number; page: number; seasons: Season[]; profileUrl: string }) {
 return <section className="panel"><div className="section-heading"><h2>Activity timeline</h2><span className="text-xs text-muted-foreground">All seasons · {count} events</span></div>
  {rows.length ? <ol className="divide-y divide-border">{rows.map(event => <li key={event.id} className="p-6">
   ...
   <dl className="mt-4 space-y-2">{Object.entries(event.changes).map(([field,change]) => <div key={field} className="text-sm"><dt className="font-medium">{fieldLabels[field] ?? field}</dt><dd className="mt-1 whitespace-pre-wrap break-words text-muted-foreground">{field === "owner_id" ? change.from_label ?? (change.from ? "Leadership member" : "Unassigned") : change.from ?? (event.event_type.startsWith("evaluation_") && evaluationAttributes.some(attribute => attribute.key === field) ? "N/A" : "Not set")}<span className="mx-2" aria-label="changed to">→</span>{field === "owner_id" ? change.to_label ?? (change.to ? "Leadership member" : "Unassigned") : change.to ?? (event.event_type.startsWith("evaluation_") && evaluationAttributes.some(attribute => attribute.key === field) ? "N/A" : "Not set")}</dd></div>)}</dl>
