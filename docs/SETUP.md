# Setup — existing Google sign-in and Phase 7
...
Run `tests/phase-six-access.sql` and the Phase 1–5 permission scripts; all fixtures are synthetic and rolled back. Phase 5 PR #4 is now merged to main. Phase 6 PR #5 was merged into the Phase 5 branch; Phase 7 carries that source to main. Deployment uses the existing GitHub/Vercel integration.


## Phase 7 data and portfolio

Migrations `20260918190925_phase_seven_private_import.sql` and `20260918191102_phase_seven_import_validation.sql` are already applied to the same project; do not reapply them. They add non-exposed private retry receipts and a narrowly guarded importer without changing leadership approval or historical access. The second migration retains Handler's full seven-character position label. Private receipt RLS intentionally denies all ordinary clients and has no client policies or grants.

Use the [private import guide](IMPORT.md) to validate the designated CSV and prepare private SQL. Python 3.11+ is required for the script/tests; the application has no new dependency or environment variable. No real import source was supplied, so importing ten real prospects remains outstanding.

Run `tests/phase-seven-access.sql` alongside the earlier scripts; all use synthetic fixtures and roll back. The [static demo](DEMO.md) is generated by normal dev/build hooks, served at `/demo/` after deployment, and independently runnable with no private app configuration. The [architecture](ARCHITECTURE.md), [synthetic screenshots](SCREENSHOTS.md) and [case study](CASE_STUDY.md) document product behavior without real player data.

Merge the Phase 7 PR to main to deploy Phase 6 continuity plus Phase 7 demo/assets through the existing integration. Stop after Phase 7.
