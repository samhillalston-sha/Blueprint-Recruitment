...
## Phase 6 — implemented historical continuity (PR #5)
...
## Phase 7 — authorized data and portfolio

- Private CSV validation/preparation script and guarded atomic import of up to ten known-source prospects. Default validation expects ten; no automatic real import occurs without a designated private source and target season. The real batch remains pending until those are supplied and reconciled privately.
- Explicit existing-person references resolve duplicate names without overwriting shared facts. Stable source IDs, input fingerprints and private receipts make unchanged retries safe; changed imported input requires review. No seasonal defaults, projections or evaluations are inferred from missing source data.
- Independently runnable read-only static demo, with ten wholly fictional people and two sample seasons; no Auth/database client or write endpoint. This is separate from production records and does not introduce an authorization bypass.
- Updated README, architecture/import/demo docs, synthetic product screenshots and an evidence-based product case study. No unsupported adoption or performance-impact claims.
...
- Complete the authorized ten-real-prospect import when the designated private source and target season are supplied. Never invent source facts or commit player data.

## Later enhancements

Tryout-event management, roster mapping/three-year composition planning, automation and AI summaries. The Phase 7 static synthetic demo is already implemented. None is needed to make the foundation useful or ready for the next phase.
