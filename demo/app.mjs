...
function dashboard(){
 ...
 return heading('Build the next chapter.',`Recruiting at a glance for ${year}. ${year===2026?'Historical sample · Read-only.':'Sample dates use January 15, 2027.'}`)+`<div class="metrics">${metrics}</div><div class="grid">${queue('Overdue follow-ups',filter(row=>row.followUp&&row.followUp<data.today))}${queue('Upcoming follow-ups',filter(row=>row.followUp&&row.followUp>=data.today))}${queue('Missing owners',filter(row=>!row.owner))}${queue('Missing next actions',filter(row=>!row.nextAction?.trim()))}</div>`+panel('Recent activity',`<div class="content timeline"><p><strong>Illustrative activity</strong> · Demo Captain West · ${year} Season</p><p>Follow-up date: Not set → ${year}-01-18</p><p class="muted">This is invented sample activity, not a production audit event.</p></div>`);
}
...
function history(){return heading('Keep the memory. Start fresh.', 'Shared people, distinct seasons, preserved evaluations.')+panel('Season continuity',`<div class="content"><p><span class="tag">2027 · Active</span></p><p>Recruiting stages, owners, next actions, outcomes and evaluations belong to the selected season.</p><p><span class="tag">2026 · Closed</span></p><p>Earlier workflow and ratings stay visible and read-only. Closing does not invent missing outcomes or clear history.</p><p>Production leadership can confirm closure by typing the season year and add an existing person to a later active season. The new membership starts with fresh fields and no copied evaluations.</p><p><a href="#prospect/demo-1">Explore Demo Prospect 01 across seasons</a></p><p class="muted">This self-contained sample has no Google login, database, import, or write endpoint. All ten people are fictional.</p></div>`);}
function render(){
